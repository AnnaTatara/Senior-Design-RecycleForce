...
width <- sum( c(205,233,208,206,71,265,265,281,286,282,289,289,288,285,276,395) )
length <- sum( c(328,326,326,372,394,138) )

width <- round(width/46,0)
length <- round(length/46,0)
...
Loc <- round(138/46, 0)
ocCorner <- c( (length-round(138/46,0)), round(sum(c(205,233,208,206))/46,0) )

# Single office
Woffice <- round(208/46,0)
Loffice <- round(sum(c(165,138))/46,0)
officeCorner <- c( length-round((165+138)/46, 0) , round( sum( c(205,233,12)/46), 0) )

# Dock
Wdock <- round(395/46,0)
Ldock <- round(sum( c( 326,372,394,138) )/46, 0)
dockCorner <- c( round( sum( c(328,326))/46, 0), round((width-Wdock), 0) )

# Bailer
Wbailer <- round(574/46,0)
Lbailer <- round(372/46,0)
bailerCorner <- c( round( sum(c(328,326,326))/46,0), width-round((395+276+285+288)/46, 0) )
...
facility <- matrix(data=1, nrow = length, ncol=width)
